...
import { transformQueryAI } from '../services/query-transform.js';
import { getFinalAnswerAI } from '../services/final-anwer.js';
...
export async function processAnalysisJob(job: Job<AnalysisJobData>): Promise<AnalysisJobResult> {
  ...
  try {
    ...
    const transformResult = await transformQueryAI(messages);
    ...
    const finalAnswerResult = await getFinalAnswerAI(transformResult, []); // Empty context for now
 ...
 */
async function updateJobResult(jobId: string, result: any, scrapedText?: string) {
  try {
    await prisma.analysisJobs.update({
      where: { id: parseInt(jobId) },
      data: { 
 ...
 */
export async function createAnalysisJob(userId: number, input: string, inputType: 'url' | 'text'): Promise<number> {
